...
@click.option('--outfile', default=None,
              help='Output file if -p is not enabled.')
@click.option('--table', default=None,
              help='Table name used for SQL statements.')
@click.option('-p', flag_value=True, is_flag=True,
...
def cli(infile, outfile, table, p, types):
    ...
    # click.echo('input file: %s' % infile)
    # click.echo('output file: %s' % outfile)
    # click.echo('p: %s' % p)
    # click.echo('table: %s' % table)
    ...
    if ext == "csv":
        ...
        raise click.ClickException('Invalid filetype. Input file extension should be .csv, .xlsx, or .xlsm')
